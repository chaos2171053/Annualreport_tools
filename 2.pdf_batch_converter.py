...
class ConverterConfig:
    ...
    processes: Optional[int] = None  # 进程数，None表示使用保守默认值（最多4个并发）
...
class PDFDownloader:
    ...
    @classmethod
    def _is_allowed_url(cls, pdf_url: str) -> bool:
        """限制下载来源域名，避免误抓取到不受信任URL。"""
        try:
    ...
    def download(self, pdf_url: str, pdf_file_path: str) -> bool:
        ...
        """
        try:
            if not self._is_allowed_url(pdf_url):
                logging.error(f"拒绝下载非白名单链接: {pdf_url}")
                return False

            # 请求PDF文件
            with self.session.get(pdf_url, stream=True, timeout=self.timeout) as response:
                ...
                tmp_file_path = f"{pdf_file_path}.part"
    ...
    def _download_with_retry(self, pdf_url: str, pdf_file_path: str) -> bool:
        ...
        for attempt in range(1, self.config.max_retries + 1):
            ...
            if attempt < self.config.max_retries:
                ...
                backoff = min(10.0, 0.8 * (2 ** (attempt - 1)))
                time.sleep(max(0.2, backoff + random.uniform(-0.3, 0.3)))
